...
export default function RoastView() {
  ...
  useEffect(() => {
    // State passed from Results.jsx (same-session navigation) — no fetch needed
    if (state) {
      setTimeout(() => setBarsAnimated(true), 400)
      return
    }

    // Fresh load from a shared URL — fetch from DB by ID
    const controller = new AbortController()
    axios
      .get(`/api/roast/${id}`, { signal: controller.signal })
      .then(res => {
        setData(res.data)
        setLoading(false)
        setTimeout(() => setBarsAnimated(true), 400)
      ...
      .catch(err => {
        ...
        setError(err.response?.data?.error || 'Roast not found.')
        setLoading(false)
      })
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: `GitHub Roast: @${data?.user?.login}`,
          text: `"${roastHeadline}" 🔥`,
          url,
        })
        return
    ...
    <div className="min-h-screen flex items-center justify-center">
      <div className="flex gap-1.5">
        {[0, 1, 2].map(i => (
          <div
            key={i}
            className="w-2 h-2 bg-roast-accent rounded-full animate-bounce"
